...
C
C      PRINT*,'ficheiro de input:'
C      READ(*,'(A12)')INPFIL
C      print*,inpfil
C      OPEN(5,FILE=INPFIL,ACCESS='SEQUENTIAL',STATUS='OLD')
c
C      REWIND(5)
C    
...
C
      DO 20 I=1,NA
        READ(5,*) STARTN(I),ENDN(I),C(I),U(I)     
20    CONTINUE
C    
...
C
C      PRINT*,'ENTER THE INITIALIZATION DESIRED'
C      PRINT*,' <0> FOR THE DEFAULT INITIALIZATION'
C      PRINT*,' <1> FOR AUCTION INITIALIZATION'
C      READ*,CRASH
	CRASH=1
C
...
C
      ...
      DO 100 I=1,NA
        ...
        if (x(i).gt.0) then
		PRINT*,' ',STARTN(I),ENDN(I),DFLOAT(X(I)) 
        endif
100   CONTINUE
...
C
      INTEGER STARTN(MAXNA),ENDN(MAXNA),U(MAXNA),X(MAXNA),DFCT(MAXNN)
...
C     THE NETWORK (GIVEN BY FOU, NXTOU, FIN, NXTIN) NEED NOT BE CONNECTED.
C
        NSCAN=0
        NLABEL=1
        LABEL(1)=IH
